...
all_services="hostname mount-proc-sys udev filesystems local syslog depmod modules pcmcia network settime inetd portmap dropbear boa smb dhcpd qtopia bluetooth"
all_services_r="bluetooth qtopia dhcpd smb boa dropbear portmap inetd settime network pcmcia modules depmod syslog local filesystems udev mount-proc-sys hostname"

cfg_services="hostname mount-proc-sys udev filesystems local $syslog depmod modules $pcmcia $network $settime $inetd $portmap $dropbear $boa $smb $dhcpd $qtopia $bluetooth"
cfg_services_r="$bluetooth $qtopia $dhcpd $smb $boa $dropbear $portmap $inetd $settime $network $pcmcia modules depmod $syslog local filesystems udev mount-proc-sys hostname"
